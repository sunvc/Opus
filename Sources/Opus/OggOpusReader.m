...
+ (NSArray<OggOpusFrame *> *_Nullable)extractFrames:(NSData *)data {
  ...
  // ogg_sync_wrote function is used to tell the ogg_sync_state struct how many
  // bytes we wrote into the buffer.
  ...
  while (ogg_sync_pageout(&ostate, &opage) == 1) {
    ...
    while (ogg_stream_packetout(&ostream, &opacket) == 1) {
      ...
      int samples;
      /* skip header packets */
      if (headers == 1 && opacket.bytes >= 19 &&
          !memcmp(opacket.packet, "OpusHead", 8)) {
        headers++;
        continue;
      }
      if (headers == 2 && opacket.bytes >= 16 &&
          !memcmp(opacket.packet, "OpusTags", 8)) {
        headers++;
        continue;
      }

      /* get packet duration */
      samples = opus_packet_get_nb_samples(
          opacket.packet, (opus_int32)opacket.bytes, sampleRate);
      if (samples <= 0) {
        invalid++;
      ...
      [result
          addObject:[[OggOpusFrame alloc]
                        initWithNumSamples:samples
                                      data:[NSData
                                               dataWithBytes:opacket.packet
                                                      length:opacket.bytes]]];
      ...
      if (!packet)
          throw Napi::Error::New(info.Env(), "Couldn't allocate packet
      buffer.");
      ...
      Napi::Buffer<unsigned char> output = Napi::Buffer<unsigned
      char>::Copy(env, reinterpret_cast<unsigned char *>(packet), packetSize);
