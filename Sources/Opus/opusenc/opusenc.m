...
typedef struct {
  int (*id_func)(unsigned char *buf,
                 int len); /* Returns true if can load file */
...
static bool comment_init(char **comments, int *length,
...
- (instancetype)init {
  return [self initWithInputSampleRate:48000
                               bitrate:30 * 1024
                           application:OggOpusWriterApplicationAudio];
}

- (instancetype)initWithInputSampleRate:(NSInteger)inputSampleRate
                                bitrate:(NSInteger)targetBitrate {
  return [self initWithInputSampleRate:inputSampleRate
...
- (instancetype)initWithInputSampleRate:(NSInteger)inputSampleRate
                                bitrate:(NSInteger)targetBitrate
                            application:
                                (OggOpusWriterApplication)targetApplication {
  self = [super init];
  if (self != nil) {
    NSInteger resolvedInputSampleRate =
        inputSampleRate > 0 ? inputSampleRate : 48000;
    bitrate = (opus_int32)(targetBitrate > 0 ? targetBitrate : 30 * 1024);
    rate = (opus_int32)resolvedInputSampleRate;
    coding_rate = rate;
    frame_size = coding_rate / 50;
    application = targetApplication == OggOpusWriterApplicationVoip
...
- (bool)beginWithDataItem:(DataItem *)dataItem {
  ...
  // Scale the resampler complexity, but only for 48000 output because the
  // near-cutoff behavior matters a lot more at lower rates
  if (rate != coding_rate) {
    NSLog(@"Invalid rate");
    return false;
  }
  ...
  int result = OPUS_OK;
  _encoder = opus_encoder_create(coding_rate, 1, application, &result);
  if (result != OPUS_OK) {
  ...
  if (ret != OPUS_OK)
  {
      NSLog(@"Error OPUS_SET_PACKET_LOSS_PERC returned: %s",
  opus_strerror(ret)); return false;
  }*/

#ifdef OPUS_SET_LSB_DEPTH
  result = opus_encoder_ctl(
      _encoder, OPUS_SET_LSB_DEPTH(MAX(8, MIN(24, inopt.samplesize))));
  if (result != OPUS_OK) {
  ...
  // Regardless of the rate we're coding at the ogg timestamping/skip is always
  // timed at 48000.
...
- (bool)initializeEncoderForAppend {
  ...
  int result = OPUS_OK;
  _encoder =
      opus_encoder_create(coding_rate, header.channels, application, &result);
  if (result != OPUS_OK) {
    NSLog(@"Error cannot create encoder: %s", opus_strerror(result));
    return false;
  }
...
- (bool)writeFrame:(uint8_t *)framePcmBytes
  ...
  if (nb_samples != 0) {
    ...
    if (nb_samples < cur_frame_size) {
      ...
      memset(paddedFrameBytes + nb_samples * 2, 0,
             cur_frame_size * 2 - nb_samples * 2);
    }

    // Encode current frame
    nbBytes = opus_encode(_encoder, (opus_int16 *)paddedFrameBytes,
                          cur_frame_size, _packet, max_frame_bytes / 10);
  ...
  // Flush early if adding this packet would make us end up with a continued
  // page which we wouldn't have otherwise
  ...
  if (framePcmBytes != NULL) {
    ...
    if (op.e_o_s) {
       ...
       */
      op.granulepos =
          ((total_samples * 48000 + rate - 1) / rate) + header.preskip;
    }
  ...
  // If the stream is over or we're sure that the delayed flush will fire, go
  // ahead and flush now to avoid adding delay
  while (
      (op.e_o_s ||
       (enc_granulepos + (frame_size * 48000 / coding_rate) - last_granulepos >
        max_ogg_delay) ||
       (last_segments >= 255))
          ? ogg_stream_flush_fill(&os, &og, 255 * 255)
...
- (NSDictionary *)saveState {
  ...
  [state setObject:@(application) forKey:@"application"];
...
- (bool)restoreState:(NSDictionary *)state withDataItem:(DataItem *)dataItem {
  ...
  NSNumber *applicationNumber = state[@"application"];
  if (applicationNumber != nil) {
    application = applicationNumber.intValue;
  }
...
static bool comment_pad(char **comments, int *length, int amount) {
  if (amount > 0) {
    char *p = *comments;
    // Make sure there is at least amount worth of padding free, and round up to
    // the maximum that fits in the current ogg segments
