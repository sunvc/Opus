    ...
    if (OP_UNLIKELY(_of->ready_state < OP_STREAMSET)) {
      if (seekable) {
        ...
        do {
          ...
          if (OP_UNLIKELY(ret < 0))
            return ret;
          _of->links[0].serialno = cur_serialno =
              (ogg_uint32_t)_of->os.serialno;
          _of->cur_link++;
        }
          ...
          if (diff < 0) {
            ...
            OP_ALWAYS_TRUE(
                !op_granpos_add(&prev_page_gp, _of->op[0].granulepos,
                                op_get_packet_duration(_of->op[0].packet,
                                                       (int)_of->op[0].bytes)));
...
static int op_decode(OggOpusFile *_of, op_sample *_pcm, const ogg_packet *_op,
  ...
  if (ret == OP_DEC_USE_DEFAULT) {
    ...
    ret = opus_multistream_decode(_of->od, _op->packet, _op->bytes, _pcm,
                                  _nsamples, 0);
#else
    ret = opus_multistream_decode_float(
        _of->od, _op->packet, (opus_int32)_op->bytes, _pcm, _nsamples, 0);
#endif
